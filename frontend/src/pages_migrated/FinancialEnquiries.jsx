...
const FinancialEnquiries = () => {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      totalAmount: enquiries.reduce((sum, e) => {
        const amt = parseInt(e.investmentAmount.replace(/[^0-9]/g, '')) || 0;
        return sum + amt;
      }, 0),
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, "financial_enquiries"), (snapshot) => {
      ...
      console.warn("Firestore financial snapshot listener failed:", error);
    });
  ...
  const deleteEnquiry = async (id) => {
    ...
    try {
      await adminApi.deleteFinancialEnquiry(id);
      fetchEnquiries();
  ...
  const exportData = async (format) => {
    ...
    try {
      ...
      if (format === 'csv') {
        ...
        link.setAttribute("download", `financial_enquiries_${new Date().getTime()}.csv`);
        ...
        XLSX.writeFile(wb, `financial_enquiries_${new Date().getTime()}.xlsx`);
      }
  ...
  return (
    <div className="space-y-8 pb-20">
      ...
      <div className="flex flex-col gap-2">
        <h1 className="text-2xl font-black text-[#0B1F4D] tracking-tight">Financial Scheme Enquiries</h1>
        <p className="text-slate-400 text-sm font-medium">Manage investment leads and scheme interest</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          { label: 'Total Enquiries', value: stats.total, icon: FinanceIcon, color: 'text-blue-500', bg: 'bg-blue-50' },
          { label: 'Potential Investment', value: `₹${stats.totalAmount.toLocaleString()}`, icon: CircleDollarSign, color: 'text-[#C9A13B]', bg: 'bg-[#C9A13B]/10' },
          { label: 'Interest This Week', value: stats.thisWeek, icon: TrendingUp, color: 'text-emerald-500', bg: 'bg-emerald-50' },
        ].map((stat, i) => (
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] border-b border-slate-50">
                ...
                <th className="px-6 py-6">Investment Details</th>
                <th className="px-6 py-6 text-right">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-50">
              {loading ? (
                <tr><td colSpan="5" className="px-6 py-32 text-center"><Loader2 size={40} className="text-[#C9A13B] animate-spin mx-auto" /></td></tr>
              ) : filteredEnquiries.length === 0 ? (
                <tr><td colSpan="5" className="px-6 py-32 text-center font-bold text-slate-300 uppercase">No financial leads found</td></tr>
              ) : (
                filteredEnquiries.map((enquiry) => (
                  <tr key={enquiry.id} className="hover:bg-slate-50/50 transition-all group">
                    <td className="px-6 py-6">
                      <div className="flex flex-col">
                        <span className="text-xs font-black text-[#0B1F4D] uppercase">{new Date(enquiry.createdAt).toLocaleDateString('en-GB')}</span>
                        <span className="text-[10px] text-slate-400 flex items-center gap-1 font-bold">
                          <Clock size={10} /> {new Date(enquiry.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                        </span>
                      </div>
                    </td>
                    <td className="px-6 py-6 text-xs font-black text-[#0B1F4D] uppercase tracking-tighter">{enquiry.fullName}</td>
                    <td className="px-6 py-6">
                      <div className="flex flex-col gap-1">
                        <div className="text-[11px] font-black text-blue-600 flex items-center gap-1">
                          <Phone size={10} /> {enquiry.phoneNumber}
                        </div>
                        <div className="text-[10px] text-slate-500 flex items-center gap-1 font-medium">
                          <Mail size={10} /> {enquiry.email}
                        </div>
                        <div className="text-[10px] text-[#C9A13B] font-black flex items-center gap-1 uppercase">
